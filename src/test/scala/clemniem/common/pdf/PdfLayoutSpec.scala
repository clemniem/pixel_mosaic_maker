...
class PdfLayoutSpec extends FunSuite {
  ...
  test("coverInstructions contains exactly one PageSize") {
    val list      = PdfLayout.coverInstructions("My Mosaic", 0, 0, PdfLayoutConfig.default)
  ...
  test("coverInstructions contains at least one Text with the title") {
    val title = "My Mosaic"
    val list  = PdfLayout.coverInstructions(title, 0, 0, PdfLayoutConfig.default)
  ...
  test("coverInstructions does not contain Save") {
    val list = PdfLayout.coverInstructions("X", 0, 0, PdfLayoutConfig.default)
