...
/** Shared palette strip UI: NES-style row of color swatches. Use previewInline for display-only, or swatches inside a button for clickable. */
object PaletteStripView {
  ...
  /** NES-style palette strip that is not clickable (same look as gallery palette button). Use for upload preview and palettes gallery. */
