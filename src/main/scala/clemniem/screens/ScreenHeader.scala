...
object ScreenHeader {
  ...
  /** Build the standard header (title, button row, optional name row).
    * @param title
    *   Screen title (e.g. "Build config", "Palette")
    * @param buttonRow
    *   HTML for the action buttons (Back, Save, etc.)
    * @param nameRow
    *   Optional name input row (entity name); use [[nameRowInput]] to build it
    * @param shortHeader
    *   If true, add screen-header--short for tighter margin
    */
  def apply[Msg](
    title: String,
    buttonRow: Html[Msg],
