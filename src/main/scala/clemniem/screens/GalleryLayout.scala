...
import clemniem.common.Pagination
...
object GalleryLayout {
  ...
  /** Total number of pages for a given list size and page size. Delegates to [[Pagination]]. */
  def totalPagesFor(size: Int, pageSize: Int): Int =
    Pagination.totalPagesFor(size, pageSize)

  /** Clamp current page to valid range [1, totalPages]. Delegates to [[Pagination]]. */
  def clampPage(current: Int, totalPages: Int): Int =
    Pagination.clampPage(current, totalPages)

  /** Slice of list for the given page, plus clamped page and total pages. Delegates to [[Pagination]]. */
  def sliceForPage[A](list: List[A], currentPage: Int, pageSize: Int): (List[A], Int, Int) =
    Pagination.sliceForPage(list, currentPage, pageSize)
  ...
  def apply[Msg](
    ...
    div(`class` := s"${NesCss.screenContainer} screen-container--gameboy")(
