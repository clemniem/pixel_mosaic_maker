...
/** Workaround for drawing on a canvas before it is in the DOM (see gbcamutil):
  ...
  *   - Retry until the element is found. Use [[drawAfterViewReady]] for all canvas updates so drawing is consistent.
  */
object CanvasUtils {
  ...
  /** Run `io` after `n` animation frames. Use n=1 for gallery list canvases (faster); use n=2 if the canvas is not
    * found yet.
    */
  ...
  /** Like [[drawAfterViewReady]] but waits for `framesToWait` frames before retrying. Use for canvases in lists (e.g.
    * gallery previews) where the element may appear later than the first frame.
    */
  def drawAfterViewReadyDelayed(
    ...
    maxRetries: Int,
    delayMs: Int
  )(draw: (Canvas, CanvasRenderingContext2D) => Unit
  ): IO[Unit] =
  ...
  /** Compute scale factor, scaled dimensions, and centering offsets for fitting srcW×srcH inside destW×destH.
    * @param maxScale
    *   upper bound on scale (e.g. 1.0 to avoid upscaling; Double.MaxValue for no limit)
    */
