...
- **Press Start 2P font (page and PDF):** The same **static asset** is used for both the website and the PDF. **`assets/PressStart2P-Regular.ttf`** is served from the app origin. The **page** loads it via **`@font-face`** in **`css/style.css`** (`url("../assets/PressStart2P-Regular.ttf")`) with **`font-display: swap`**, so no font-loading script or `visibility: hidden` workaround is needed; content is visible immediately and the font swaps in when ready. The **PDF** (jsPDF) fetches the same URL at generation time, converts the response to base64, and registers it with **`doc.addFileToVFS`** / **`doc.addFont`** / **`doc.setFont("Press Start 2P", "normal")`** in **`JsPDF.scala`** so the printed book uses the same typeface. If the font fetch fails (e.g. offline), the PDF still generates with the default font.
...
- **Font:** The PDF uses the same **Press Start 2P** font as the page; see **§8** (static asset, fetch + addFileToVFS/addFont in `JsPDF.scala`).
...
- **Deploy index.html is generated, not copied:** The workflow **generates** `dist/index.html` (see the heredoc in **`.github/workflows/deploy.yml`**). It does **not** copy the repo’s `index.html`. So any change to the **local** `index.html` (e.g. new scripts or link tags) must be **replicated in the workflow** when generating `dist/index.html`. The font is loaded via **`@font-face`** in `css/style.css` from the same static asset as the PDF (§8); there is no font-loading script in the generated HTML.
