...
- **Library:** The app uses **[jsPDF](https://github.com/parallax/jsPDF)** for client-side PDF generation (print instructions book). See the **Libraries** table in the README.
- **Font:** The PDF uses the same **Press Start 2P** font as the page; see **§8** (fontData.js → `window.pressStartBase64`, addFileToVFS/addFont/setFont in `JsPDF.scala`).
...
- **Deploy index.html is generated, not copied:** The workflow **generates** `dist/index.html` (see the heredoc in **`.github/workflows/deploy.yml`**). It does **not** copy the repo’s `index.html`. So any change to the **local** `index.html` (e.g. new scripts or link tags) must be **replicated in the workflow** when generating `dist/index.html`. The font is loaded via **`@font-face`** in `css/style.css` (page) and via **`fontData.js`** (PDF; see §8). The generated HTML includes **`<script src="./assets/fontData.js"></script>`** before the app; there is no other font-loading script.
...
- **Service worker cache version:** The app registers a service worker in production (**`sw-register.js`** only registers when not localhost). The worker (**`sw.js`**) caches HTML, CSS, and fetches; the **cache name** is **`CACHE_NAME`** (e.g. `pixel-mosaic-maker-v3`). If users keep seeing an old version after a deploy (e.g. new features like the theme toggle don’t appear), **bump the cache version** in **`sw.js`** (e.g. `v3` → `v4`). On activate, the new worker deletes all other caches, so the next load fetches fresh assets. The worker uses **`self.skipWaiting()`** on install so the new worker takes over without closing tabs. If the issue persists, check CDN or server caching of `index.html` and `sw.js`.
