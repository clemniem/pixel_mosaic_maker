...
- **Font FOUT (flash of unstyled text):** To avoid a brief flash of fallback font before “Press Start 2P” loads, **`body`** starts with **`visibility: hidden`** in CSS; a script (in **`index.html`** and in the **deploy workflow’s** generated `index.html`) adds a class so content becomes visible. The script must add the class to **`document.documentElement`** (the `html` element), **not** `document.body`, because it runs in **`<head>`** before `<body>` exists. CSS uses **`html.fonts-loaded body { visibility: visible }`**. The script uses `document.fonts.ready` and a 3s timeout fallback so content always appears.
...
- **Deploy index.html is generated, not copied:** The workflow **generates** `dist/index.html` (see the heredoc in **`.github/workflows/deploy.yml`**). It does **not** copy the repo’s `index.html`. So any change to the **local** `index.html` (e.g. font preconnect, preload, or the font-loading script) must be **replicated in the workflow** when generating `dist/index.html`. If the font-loading script is missing from the generated HTML, the deployed site still loads `css/style.css` with `body { visibility: hidden }` and no script adds `fonts-loaded` to the document; the page then shows **only the gray background** and no content. The generated index must include the same script that adds the class to **`document.documentElement`** (see §8 font FOUT).
